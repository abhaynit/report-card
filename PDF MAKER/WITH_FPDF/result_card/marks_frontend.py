...
def call_fun():
    ...
    Label(root, text = "MARKS WINDOW",	background = 'green', foreground ="white",font = ("Times New Roman", 15)).pack()
    ...
    def subject_list(): 
        ...
        for i in final_value:
            ...
            x_val+=190 
            count+=1
        Button(root, text = "GO".center(20,' '),state=DISABLED, command = subject_list).place(x=240,y=210)
        ...
        def select():
            from student_registration import marks_addition

            if marks_addition(reg_no.get(),se.get(),mo.get()+' '+yea.get(),sub1.get(),sub2.get(),sub3.get(),sub4.get(),sub5.get(),sub6.get(),sub7.get(),sub8.get(),sub9.get()):
        ...
        Button(root, text = "ENTER".center(20,' '),command = select).place(x=200,y=y_vaL+50)
        Button(root, text = "UPDATE ANOTHER".center(20,' '),command = update_another).place(x=300,y=y_vaL+50)
    Button(root, text = "GO".center(20,' '),command = subject_list).place(x=240,y=210)
